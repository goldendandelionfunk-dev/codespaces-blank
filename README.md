<div align="center">
  ...
  <hr />
  Anonub (Anonymous Unblocker) version 5 is finally here!
...
Anonub is a web proxy frontend / internet browsing hub, allowing you to browse the web anonymously while providing a full suite of apps and games, built with [React](https://github.com/facebook/react).
...
#### Deploying with Docker (not recommended):

> ⚠️ Docker images often add complexity and may expose the proxy to
> additional attack surfaces. For privacy‑focused, local use we recommend
> running the built Javascript bundle via `node server.js` or using the `npm`
> scripts instead.
...
docker run -d \
  ...
  -e PORT=3000 \
  ghcr.io/dogenetwork/dogeub:latest # replace with anonub image when published
```
