...
export const privacyConfig = ({ options, updateOption, openPanic }) => ({
  0: {
    ...
    desc: 'Type a custom name to impersonate another site (overrides selection above).',
    value: options.tabName || '',
    type: 'input',
    action: (val) => {
      updateOption({ tabName: val });
      import('/src/utils/utils.js').then(({ ckOff }) => ckOff());
    },
  },
  1: {
    name: 'Custom Tab Icon URL',
    desc: 'A URL to the favicon you want to use (png/svg/ico).',
    value: options.tabIcon || '',
    type: 'input',
    action: (val) => {
      updateOption({ tabIcon: val });
      import('/src/utils/utils.js').then(({ ckOff }) => ckOff());
    },
  },
  2: {
